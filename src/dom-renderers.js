...
function dashboardMain(state, reviewed) {
  ...
  return `${screenHeading('dashboard', '01 / 06 · intake', 'One bounded case. Three governed fields. A visible human decision boundary.')}
...
function signoffMain(state, { conflictDecided, canSignOff }) {
  ...
  const disabled = conflictDecided ? '' : ' disabled';
  return `${screenHeading('review-signoff', '05 / 06 · gate', 'Inspect the human decision ledger before producing a local evidence receipt.')}
    ...
    <section class="officer-confirmation ${canSignOff ? 'gate-cleared' : 'gate-blocked'}" aria-labelledby="officer-confirmation-title">
      ...
      <p>${conflictDecided
        ...
        : 'Blocked until the conflict decision above is recorded.'}</p>
...
export function renderMain(route, state, { reviewed, conflictDecided, canSignOff, manifest }) {
  ...
  if (route === 'review-signoff') return signoffMain(state, { conflictDecided, canSignOff });
